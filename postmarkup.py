...
class Forum(db.Model, DataBaseOperations):
...
class Post(db.Model, DataBaseOperations):
  ...
  def googleTalkStatus(self):
    try:
      status = xmpp.get_presence(self.author.email())
    except:
      status = False
    return status
  ...
  def sendMailToAll(self, sender, additionalText):
    ...
    if len(toEmails) != 0:
      ...
      message.html = self.body + additionalText
      message.send()
    #print 'mailing OK'
